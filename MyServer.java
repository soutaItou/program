...
class  extends Thread {
	...
	private Socket incoming;//接続されていないソケット
	private InputStreamReader myIsr;//バイトストリームから文字ストリームへの橋渡し
	private BufferedReader myIn;//文字をバッファリングして効率よく読み込む
	private PrintWriter myOut;//テクスト出力ストリームに出力
	...
	public void run() {
		...
		try {
			myOut.println("Mynumber"+" "+ number);//初回だけ呼ばれる
	...
	public static void main(String[] args) {
		...
		try {
			...
			while (true) {
				...
				incoming[n] = server.accept();//接続があるまで待ち続けます
				flag[n] = true;
